...
export const SmartEcosystem: React.FC = () => {
  ...
  const DeviceNode: React.FC<{ 
    ...
    <div className={`absolute ${position} flex flex-col items-center transition-all duration-500 z-20`}>
      <div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center bg-zinc-900 shadow-2xl transition-all duration-500 ${
        status !== 'idle' 
          ? 'border-brand-green shadow-[0_0_30px_rgba(16,185,129,0.3)] scale-110' 
          : 'border-zinc-700 opacity-70 grayscale'
      }`}>
        <Icon className={`${status !== 'idle' ? color : 'text-zinc-500'} w-8 h-8 md:w-10 md:h-10 transition-colors duration-300`} />
      </div>
      <div className={`mt-3 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md border ${
        status !== 'idle' ? 'bg-brand-green/10 border-brand-green text-brand-green' : 'bg-zinc-800/50 border-zinc-700 text-zinc-500'
      }`}>
        {label}
      </div>
    </div>
  ...
  const ConnectionLine: React.FC<{ 
    ...
    return (
      <div className={`absolute ${getStyle()} bg-zinc-800 z-0 overflow-hidden`}>
        {active && (
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <h4 className={`font-bold ${isActive ? 'text-brand-green' : 'text-white group-hover:text-zinc-200'}`}>
                      {s.title}
                    </h4>
          ...
          <div className="lg:col-span-8 order-1 lg:order-2">
            <Reveal>
              <div className="relative aspect-square md:aspect-[4/3] bg-zinc-900/30 rounded-full border border-zinc-800 shadow-inner flex items-center justify-center max-w-2xl mx-auto">
                
                {/* Lines - Simplified for Layout */}
                <div className="absolute inset-0 opacity-20">
                   <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-zinc-500" />
                   <div className="absolute top-0 bottom-0 left-1/2 right-1/2 w-0.5 bg-zinc-500" />
                   <div className="absolute top-1/2 left-1/2 w-full h-0.5 bg-zinc-500 origin-left rotate-45" />
                </div>
                ...
                <div className="relative z-30 w-24 h-24 bg-zinc-800 rounded-full border-4 border-zinc-600 flex items-center justify-center shadow-2xl">
                  <Home size={40} className="text-white" />
                  <div className="absolute -bottom-8 text-xs font-bold text-zinc-500 uppercase tracking-widest">Inverter</div>
                </div>
